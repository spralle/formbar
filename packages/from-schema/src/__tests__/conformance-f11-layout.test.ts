...
describe("F11.D: Unknown node rejection", () => {
	test("F11.11: Registry validate() throws FORMBAR_LAYOUT_UNKNOWN_NODE_TYPE for unknown node type", () => {
		const registry = new LayoutNodeRegistry();
		const tree: LayoutNode = {
			type: "group",
			id: "root",
		...
		try {
			...
			expect((e as FromSchemaError).code).toBe("FORMBAR_LAYOUT_UNKNOWN_NODE_TYPE");
		}
	});
});
